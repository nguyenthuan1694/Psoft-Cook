<section class="service section-bg fill bg-fill bg-loaded">
        <div class="container">
            <div class="mt-5 mb-5">
                <h3 class="text-uppercase text-center font-weight-bold" style="color: #FFF">Dịch vụ nỗi bật của chúng tôi</h3>
            </div>
            <div class="row">
                <div class="col-lg-6 col-xs-12 left mb-5">
                    <div class="product-service__item">
                        <div class="product-service__img">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chat-right-text" viewBox="3 -9 10 30">
                                <path d="M2 1a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h9.586a2 2 0 0 1 1.414.586l2 2V2a1 1 0 0 0-1-1H2zm12-1a2 2 0 0 1 2 2v12.793a.5.5 0 0 1-.854.353l-2.853-2.853a1 1 0 0 0-.707-.293H2a2 2 0 0 1-2-2V2a2 2 0 0 1 2-2h12z"/>
                                <path d="M3 3.5a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5zM3 6a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9A.5.5 0 0 1 3 6zm0 2.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5z"/>
                            </svg>
                        </div>
                        <div class="bl-info">
                            <h5>FREE SHIPPING</h5> 
                            <p>Shop sẽ có dịch vụ free ship cho khách hàng khi <br> miễn phí</p> 
                            <button class="btn btn-primary w-100">Tư vấn ngay</button>
                        </div>
                    </div>
                </div>
                <div class="col-lg-6 col-xs-12 right mb-5">
                    <div class="product-service__item">
                        <div class="product-service__img">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-telephone-inbound" viewBox="3 -9 10 30">
                                <path d="M15.854.146a.5.5 0 0 1 0 .708L11.707 5H14.5a.5.5 0 0 1 0 1h-4a.5.5 0 0 1-.5-.5v-4a.5.5 0 0 1 1 0v2.793L15.146.146a.5.5 0 0 1 .708 0zm-12.2 1.182a.678.678 0 0 0-1.015-.063L1.605 2.3c-.483.484-.661 1.169-.45 1.77a17.568 17.568 0 0 0 4.168 6.608 17.569 17.569 0 0 0 6.608 4.168c.601.211 1.286.033 1.77-.45l1.034-1.034a.678.678 0 0 0-.063-1.015l-2.307-1.794a.678.678 0 0 0-.58-.122l-2.19.547a1.745 1.745 0 0 1-1.657-.459L5.482 8.062a1.745 1.745 0 0 1-.46-1.657l.548-2.19a.678.678 0 0 0-.122-.58L3.654 1.328zM1.884.511a1.745 1.745 0 0 1 2.612.163L6.29 2.98c.329.423.445.974.315 1.494l-.547 2.19a.678.678 0 0 0 .178.643l2.457 2.457a.678.678 0 0 0 .644.178l2.189-.547a1.745 1.745 0 0 1 1.494.315l2.306 1.794c.829.645.905 1.87.163 2.611l-1.034 1.034c-.74.74-1.846 1.065-2.877.702a18.634 18.634 0 0 1-7.01-4.42 18.634 18.634 0 0 1-4.42-7.009c-.362-1.03-.037-2.137.703-2.877L1.885.511z"/>
                            </svg>
                        </div>
                        <div class="bl-info"> 
                            <h5>30 DAYS RETURN</h5> 
                            <p>Tìm kiếm theo tiêu chí, đảm bảo an toàn pháp lý</p> 
                            <button class="btn btn-primary w-100">Liên hệ ngay</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
<footer id="footer">
    <div class="container section--default pt-5">
        <div class="row">
            <div class="col-lg-6 col-md-5 efch-1 ef-img-t">
                <div class="widget widget-info">
                    <p> 
                        <a href="/" class="logo logo-footer"> 
                            <img style="width: 162px; height: 83px;" data-lazy-type="image" alt="Propzy" src="{{ asset('frontend/images/logos/logo P-SOFT.svg') }}"> 
                        </a> 
                    </p>
                    <p>Số 296 Hòa Bình, Phường Hiệp Tân, Quận Tân Phú, Thành phố Hồ Chí Minh</p>
                    <p>Email: nguyenthuan1694@gmail.com</p>
                    <p>Phone: 090.8855.483</p>
                </div>
            </div>
            <div class="col-lg-6 col-md-7 efch-1 ef-img-t">
                <div class="row">
                    <div class="col-lg-4 col-sm-6 col-xs-12">
                        <div class="footer__contract">
                            <div class="widget-title text-uppercase">
                                Về chúng tôi
                            </div>
                            <ul class="footer__contract-content" style="font-size: 14px">
                                <li><a href="{{ route('introduce') }}">Giới thiệu</a></li>
                                <li><a href="{{ route('news-of-event') }}">Tin tức và sự kiện</a></li>
                                <li><a href="#">Liên hệ</a></li>
                                <li><a href="#">Chính sách bảo mật</a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-4 col-sm-6 col-xs-12">
                        <div class="footer__contract">
                            <div class="widget-title text-uppercase">
                                    Dịch vụ của PSoft
                                </div>
                                <ul class="footer__contract-content" style="font-size: 14px">
                                    <li><a href="#">Gửi BDS miễn phí</a></li>
                                    <li><a href="#">Thông tin huy hoạch</a></li>
                                    <li><a href="#">Thẩm định giá BDS</a></li>
                                    <li><a href="#">Thủ tục vay mua nhà</a></li>
                                </ul>
                        </div>
                    </div>
                    <div class="col-lg-4 col-sm-6 col-xs-12">
                        <div class="footer__contract">
                            <div class="widget-title text-uppercase">
                                hệ thống của PSoft
                            </div>
                            <ul class="footer__contract-content" style="font-size: 14px">
                                <li><a href="#">Mua nhà</a></li>
                                <li><a href="#">Bán nhà</a></li>
                                <li><a href="#">thuê nhà</a></li>
                                <li><a href="#">Cộng đồng môi giới Psoft</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="line"></div>
        <div class="row mb-5">
            <div class="col-lg-3 col-sm-3">
                <div class="footer-social__title">
                    kết nối với chúng tôi
                </div>
                <ul class="blog-item-social">
                    <li>
                        <a href="#">
                            <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" fill="currentColor" class="bi bi-facebook" viewBox="0 0 16 16">
                                <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z"/>
                            </svg>
                        </a>
                    </li>
                    <li>
                        <a href="#">
                            <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" fill="currentColor" class="bi bi-twitter" viewBox="0 0 16 16">
                                <path d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334 0-.14 0-.282-.006-.422A6.685 6.685 0 0 0 16 3.542a6.658 6.658 0 0 1-1.889.518 3.301 3.301 0 0 0 1.447-1.817 6.533 6.533 0 0 1-2.087.793A3.286 3.286 0 0 0 7.875 6.03a9.325 9.325 0 0 1-6.767-3.429 3.289 3.289 0 0 0 1.018 4.382A3.323 3.323 0 0 1 .64 6.575v.045a3.288 3.288 0 0 0 2.632 3.218 3.203 3.203 0 0 1-.865.115 3.23 3.23 0 0 1-.614-.057 3.283 3.283 0 0 0 3.067 2.277A6.588 6.588 0 0 1 .78 13.58a6.32 6.32 0 0 1-.78-.045A9.344 9.344 0 0 0 5.026 15z"/>
                            </svg>
                        </a>
                    </li>
                    <li>
                        <a href="#">
                            <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" fill="currentColor" class="bi bi-youtube" viewBox="0 0 16 16">
                                <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.007 2.007 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.007 2.007 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31.4 31.4 0 0 1 0 7.68v-.122C.002 7.343.01 6.6.064 5.78l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.007 2.007 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A99.788 99.788 0 0 1 7.858 2h.193zM6.4 5.209v4.818l4.157-2.408L6.4 5.209z"/>
                            </svg>
                        </a>
                    </li>
                    <li>
                        <a href="#">
                            
                        </a>
                    </li>
                </ul>
            </div>
            <div class="col-lg-3 col-sm-3">
               <a href="#">
                   <img style="width: 186px; height: 70px;" src="{{ asset('frontend/images/logos/img-dathongbao.png') }}" alt="">
               </a>
            </div>
            <div class="col-lg-4 col-sm-4">
                <div class="footer-social__title">
                    Đăng ký nhận thông tin mới nhất từ chúng tôi
                </div>
                <div class="form-group">
                    <input type="text" class="form-control" placeholder="Nhập email để nhận thông tin" aria-label="Username" aria-describedby="basic-addon1">
                    <button class="btn--register btn-orange footer__register">
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-arrow-right-short" viewBox="0 0 16 16">
                            <path fill-rule="evenodd" d="M4 8a.5.5 0 0 1 .5-.5h5.793L8.146 5.354a.5.5 0 1 1 .708-.708l3 3a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708-.708L10.293 8.5H4.5A.5.5 0 0 1 4 8z"/>
                        </svg>
                    </button>
                </div>
            </div>
            <div class="col-lg-2 col-sm-2"></div>
        </div>
    </div>
</footer>
<div class="copyright">
    2021 © Copyright Psoft. All rights reserved.
</div>
<div class="bl-chat">
    <div class="bl-a-chat">
        <div class="phonering-alo-phone phonering-alo-green phonering-alo-show" id="phonering-alo-phoneIcon">
            <div class="phonering-alo-ph-circle"></div>
            <div class="phonering-alo-ph-circle-fill"></div>
            <a href="tel:+0908855483" class="pps-btn-img" title="Liên hệ">
                <div class="phonering-alo-ph-img-circle"></div>
            </a>
        </div>
        <a class="bl-social" href="#">
            <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="currentColor" class="bi bi-chat-dots-fill" viewBox="4 -10 1 30">
                <path d="M16 8c0 3.866-3.582 7-8 7a9.06 9.06 0 0 1-2.347-.306c-.584.296-1.925.864-4.181 1.234-.2.032-.352-.176-.273-.362.354-.836.674-1.95.77-2.966C.744 11.37 0 9.76 0 8c0-3.866 3.582-7 8-7s8 3.134 8 7zM5 8a1 1 0 1 0-2 0 1 1 0 0 0 2 0zm4 0a1 1 0 1 0-2 0 1 1 0 0 0 2 0zm3 1a1 1 0 1 0 0-2 1 1 0 0 0 0 2z"/>
            </svg>
        </a>
    </div>
</div>